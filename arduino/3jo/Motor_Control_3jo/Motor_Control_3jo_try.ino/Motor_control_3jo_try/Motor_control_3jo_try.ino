/* 3조 모터컨트롤 코드*/
/* 엔코더 부분 */
/*8월 11일 get_data 내부 while문 제거,  mt_ctrl_cnt 모두 주석처리*/
...
int is_braking = 0; // 브레이크 모터가 동작 중인가? 0: 브레이크모터 동작 X / 1: 브레이크모터 동작 O
  ...
  /* 엔코더 조향각이 바뀔 때마다 steer_angle입력을 막기 위해 while문 추가*/
   ...
   int change = getEncoderTurn(); // encoder 각도 변화량
    ...
    if (encoderVal <= degree) // 방향 제어
      ...
      digitalWrite(mt_DIR, HIGH); // CCW 방향
    digitalWrite(mt_SPEED,255);
